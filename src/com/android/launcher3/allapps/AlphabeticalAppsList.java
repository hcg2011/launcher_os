...
import com.android.launcher3.UsedAppsTool;
...
//prize add by zhouerlong 20190116
        if(Launcher.Usage) {
            if (mApps != null && mApps.size() > 0) {
                UsedAppsTool t = new UsedAppsTool(mApps, mLauncher);
                List<UsageTable> items = t.find();
                List<AppInfo> usages = t.ft(items);
                List<AppInfo> exits = new ArrayList<>(mApps);
                exits.remove(usages);
                mApps.clear();
