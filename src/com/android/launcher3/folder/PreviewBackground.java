...
import com.android.launcher3.theme.tools.ImageUtils;
import com.android.launcher3.theme.tools.ThemeIconTool;
import com.android.launcher3.util.BitmapUtil;
//add by zhouerlong prize launcher 20180906
...
//add by zhouerlong prize launcher 20180906
        if(LauncherAppState.isDisableAllApps()) {
            this.previewSize = grid.iconSizePx;
        }else {
...
//add by zhouerlong prize launcher 20180906
    ...
    private void drawBoundRect(Canvas canvas) {
        mRectF.left = basePreviewOffsetX;
        mRectF.top = basePreviewOffsetY;
        mRectF.right = mRectF.left + previewSize;
        mRectF.bottom = mRectF.top + previewSize;
        mPaint.setColor(Color.argb(100, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY));
        canvas.drawRoundRect(mRectF.left, mRectF.top, mRectF.right, mRectF.bottom, 10, 10, mPaint);
    }
    public void drawThemeBackground(Canvas canvas, Launcher launcher) {
        Bitmap bg = ThemeIconTool.getInstance().getFolderIcon(launcher.getApplicationContext());
        ...
        if(bg!=null) {
            bg = ImageUtils.resize(bg,previewSize,previewSize);
        }
        ...
        if (width > previewSize || height > previewSize) {
            bg = BitmapUtil.zoomImg(bg, previewSize, previewSize);
        }
        ...
        mPaint.setColor(Color.argb(250, BG_INTENSITY, BG_INTENSITY, BG_INTENSITY));

//        canvas.drawBitmap(bg, null, mRectF, null);
        canvas.scale(mScale,mScale,mRectF.left+mRectF.width()/2,mRectF.top+mRectF.height()/2);
        canvas.drawBitmap(bg,mRectF.left,mRectF.top,mPaint);
    }
//add by zhouerlong prize launcher 20180906
