...
import com.android.launcher3.PrizeDragView;
...
//add by zhouerlong prize launcher 20180906
        final DragView dragView = mDragObject.dragView = new PrizeDragView(mLauncher, b, registrationX,
...
//add by zhouerlong prize launcher 20180906
	...
	//add by zhouerlong 20181130  double point draging
        if(!(mLastDropTarget instanceof DeleteDropTarget)&&ev.getPointerCount()>1&&Launcher.DOUBLE_FINGER_SLIDING_PAGE) {

            try {
               /* if (((Workspace) mLastDropTarget).getDragInfo().cell instanceof BubbleTextView) {
                    if (mOptions.preDragCondition != null
                            && mOptions.preDragCondition.shouldStartDrag(mDistanceSinceScroll)){
