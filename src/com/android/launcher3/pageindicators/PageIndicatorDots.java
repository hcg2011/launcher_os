...
//add by zhouerlong prize launcher 20180906
    ...
    public PageIndicatorDots(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		//prize add by huhuan, three points,20180928-start
        mActiveColor = context.getResources().getColor(R.color.page_indicator_dots_select_color);//Themes.getColorAccent(context);
        mInActiveColor = context.getResources().getColor(R.color.page_indicator_dots_color);//Themes.getAttrColor(context, android.R.attr.colorControlHighlight);
       //prize add by huhuan, three points ,20180928-end
    ...
    public void setScroll(int currentScroll, int totalScroll) {
        if (mNumPages > 1) {
            ...
            //prize add by huhuan,Solve AndroidRuntime,20181012-start
            int pageToLeft;
            if(scrollPerPage == 0){
                pageToLeft = 0;
            }else {
            ...
            //prize add by huhuan,Solve AndroidRuntime,20181012-end
...
//prize add by huhuan, three points,20180928-end
    ...
    protected void onDraw(Canvas canvas) {
		...
		//prize add by huhuan, change points distance, 20180929-start
        float circleGap = 36;//3 * mDotRadius;
        //float startX = (getWidth() - mNumPages * circleGap + mDotRadius) / 2;
		float startX;
		if(mNumPages >= 1){
            startX = (getWidth() - (mNumPages-1) * circleGap -2* mDotRadius)  /2;
		}else{
		    startX = (getWidth() - mNumPages * circleGap + mDotRadius) / 2;
		}
		//prize add by huhuan, change points distance, 20180929-start
    ...
    private RectF getActiveRect() {
		...
		//prize add by huhuan, change points distance, 20180929-end
