...
@RemoteView
public class PrizeBubbleTextView extends DoubleShadowBubbleTextView {
    private int mDatas;
    ...
    private RectF cbgRect = new RectF();
    private RectF cugRect = new RectF();
    private RectF cdgRect = new RectF();
...
//			mSecondHand.recycle();
    ...
    public void OnDrawCalendarView(Canvas canvas) {
        ...
        if (isCalendarView) {
            ...
            float top = 0;
            float y = this.getCompoundPaddingTop() / 2f + this.getPaddingTop() / 2f-getPaddingLeft()/2;// + top;
...
//                canvas.scale(scaled, scaled, x, y);
            ...
            if (mBWeek != null) {
                ...
                float y1 = this.getCompoundPaddingTop() / 2f + this.getPaddingTop() / 2f;
    ...
    public void OnDrawDeskClockView(Canvas canvas) {
        ...
        if (isDeskClockView && isResourceOK) {
          ...
          /*  if (seconds) {
            ...
            float x = this.getWidth() / 2f;
            float y = this.getCompoundPaddingTop() / 2f + this.getPaddingTop() / 2f-getPaddingLeft()/2;
        /*	if(Launcher.scale == 3){
...
//-------------------------------------------------------------------------------------------------
    ...
    public class MyCount extends CountDownTimer {
        ...
        public void onTick(long millisUntilFinished) {
            if(isDeskClockView) {
                PrizeBubbleTextView.this.onTick();
            }
            if(isCalendarView) {
                PrizeBubbleTextView.this.onTick();
            }
    ...
    protected int[] onCreateDrawableState(int extraSpace) {
        ...
        return !isDeskClockView&&!isCalendarView?drawableState:super.onCreateDrawableState(extraSpace);
    }
}
