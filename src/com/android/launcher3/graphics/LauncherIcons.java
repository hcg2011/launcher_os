...
import android.content.Intent.ShortcutIconResource;
	//add by zhouerlong 20180906 update all apps 
...
public class LauncherIcons implements AutoCloseable {
	...
	//add by zhouerlong 20180906 update all apps 
//add by zhouerlong 20190116
        if (Launcher.src_icon&&!LauncherAppState.isDisableAllApps()&&Utilities.ATLEAST_OREO /*&& iconAppTargetSdk >= Build.VERSION_CODES.O*/) {
	...
	//add by zhouerlong 20180906 update all apps 
            ...
            if (Launcher.src_icon&&Utilities.ATLEAST_OREO && !outShape[0] && !(icon instanceof AdaptiveIconDrawable)) {
//add by zhouerlong 20190116
    ...
    private Bitmap createIconBitmap(Drawable icon, float scale) {
        ...
        if (Utilities.ATLEAST_OREO && icon instanceof AdaptiveIconDrawable) {
		//add by zhouerlong prize 20181218 
            int offset =0;// Math.max((int) Math.ceil(BLUR_FACTOR * textureWidth), Math.max(left, top));
		//add by zhouerlong prize 20181218 
