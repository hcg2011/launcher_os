...
public abstract class PagedView<T extends View & PageIndicator> extends ViewGroup {
	...
	//add by zhouerlong smooth 
    private static final float DELTAX_VELOCITYTRACKER_MAX = 1.45f;
	//add by zhouerlong smooth 
    //add by zhouerlong 20181128
    ...
    public FLING_STATE mFlingState= FLING_STATE.NONE;
	//add by zhouerlong 20181128
	...
	//add by zhouerlong 20181128
                if(isFlingDown()&&ev.getPointerCount()<=1) {
                    expandSystemUI();
                }
	//add by zhouerlong 20181128
	...
	//add by zhouerlong   move not to -1 screen 20190123
	//add by zhouerlong 20181128
    private boolean isScrollY(int xDiff, int yDiff, int touchSlop) {
        boolean ySin = Math.abs(xDiff/(float)yDiff) < Math.sin(Math.PI*45/180);
        if (ySin) {
            return true;//FLING_STATE.DOWN;//乡下滑动
        }  else {
            return false;
        }
    }
    ...
    public boolean isFlingDown(){
        return mFlingState == FLING_STATE.DOWN;
    }

    /**y轴快速下拉弹出systemUI托盘
     */
    protected void  expandSystemUI() {
	//add by zhouerlong 20181130  double point draging
    ...
    public void expand() {
        try {
            Object service = this.getContext().getSystemService("statusbar");
	...
	//add by zhouerlong 20181130  double point draging
    private void handleMaskPointDwonEvnet(MotionEvent ev) {
        if (mLauncher!=null&&mLauncher.getDragController()!=null&&mLauncher.getDragController().isDragging()&&ev.getPointerCount()>1) {
            mActivePointerId = ev.getPointerId(1);
            if (!mScroller.isFinished()) {
                abortScrollerAnimation(false);
            }

            mDownMotionX = mLastMotionX = ev.getX(mActivePointerId);
            mLastMotionY = ev.getY(mActivePointerId);
            mDownMotionY = ev.getY(mActivePointerId);
            mLastMotionXRemainder = 0;
            mTotalMotionX = 0;
            mActivePointerId = ev.getPointerId(0);
	...
	//add by zhouerlong 20181130  double point draging
        if (!isTouchPointInViewportWithBuffer((int) x, (int) y)&& ev.getPointerCount()<=1) return;
	//add by zhouerlong 20181130  double point draging
        ...
        boolean  isTouchDown = y - mLastMotionY>0;
	//add by zhouerlong 20181130  double point draging

	//add by zhouerlong   move not to -1 screen 20190123
       boolean isScrollY = isScrollY(xDiff,yDiff,touchSlop )&&ev.getPointerCount()<=1;
	//add by zhouerlong 20181130  double point draging
		//add by zhouerlong 20181128

        boolean xMoved = xDiff > touchSlop&&!isScrollY;
	//add by zhouerlong 20181130  double point draging error
        boolean yMoved = yDiff >touchSlop*2;
	//add by zhouerlong 20181130  double point draging error
        if (xMoved) {
	...
	//add by zhouerlong 20181130  double point draging
            ...
            if(ev.getPointerCount()>1) {
                mTouchState = TOUCH_STATE_REST; //表示状态为滑动状态
                //add by zhouerlong 在 5706机型出现 拖动图标首次不能滑页
                if(mLauncher!=null&&mLauncher.getDragController()!=null&&mLauncher.getDragController().isDragging()) {
                    handleMaskPointDwonEvnet(ev);
                }
                //add by zhouerlong 在 5706机型出现 拖动图标首次不能滑页
            }
        }
...
//                 float deltaX = mLastMotionX + mLastMotionXRemainder - x;
                ...
                float deltaX= DELTAX_VELOCITYTRACKER_MAX*(mLastMotionX + mLastMotionXRemainder - x);
	//add by zhouerlong smooth 
	...
	//add by zhouerlong 20181130  double point draging
    public void handleMaskPointUpEvent(MotionEvent ev) {
            int activePointerId = mActivePointerId;
