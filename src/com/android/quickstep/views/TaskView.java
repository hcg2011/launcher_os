...
public class TaskView extends FrameLayout implements TaskCallbacks, PageCallbacks {
    ...
    public void onTaskDataLoaded(Task task, ThumbnailData thumbnailData) {
        ...
        if(task.getTopComponent().getPackageName().equals("com.android.phone")||task.getTopComponent().getPackageName().equals("com.android.server.telecom")) {
            String pkg = "com.android.dialer";
             com =  new ComponentName(pkg,ThemeIconTool.getInstance().getAppMainActivity(getContext(),pkg));

        }else if(task.getTopComponent().getPackageName().equals("com.android.settings.intelligence")){
            String pkg = "com.android.settings";
            com =  new ComponentName(pkg,ThemeIconTool.getInstance().getAppMainActivity(getContext(),pkg));
        }
            else {
             com =  new ComponentName(task.getTopComponent().getPackageName(),ThemeIconTool.getInstance().getAppMainActivity(getContext(),task.getTopComponent().getPackageName()));

        }
//add prize by zhouerlong
        String p = task.getTopComponent().getPackageName();
        String cls=ThemeIconTool.getInstance().getAppMainActivity(getContext(),task.getTopComponent().getPackageName());
        Drawable f= ThemeIconTool.getInstance().getThemeIcon(com, task.icon, getContext());
...
//add prize by huhun20181130
	...
	//prize add by huhuan,without Icon when Horizontal screen Get into Recent mission,20181123-start
       // mIconView.animate().scaleX(scale).scaleY(scale).setDuration(SCALE_ICON_DURATION).start();
	  	//prize add by huhuan,without Icon when Horizontal screen Get into Recent mission,20181123-end 
